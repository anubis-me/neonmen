...
function machineGun(chunks, maxLength) {
  ...
  for (i = 0; i < chunks.length; i++) {
    ...
    if (isSentenceEnd) {
      tl.to(element, duration, {scale:1.2,  ease:SlowMo.ease.config(0.25, 0.9)}, time)
		 	  .to(element, duration, {autoAlpha:1, ease:SlowMo.ease.config(0.25, 0.9, false)}, time);
    }
    else{
      tl.to(element, duration, {scale:1.2,  ease:SlowMo.ease.config(0.25, 0.9)}, time)
      ...
      time += duration - 0.05;
      if (isSentenceEnd) {
        time += 6.0; //at the end of a sentence, add a pause for dramatic effect.
      }
    }
}

machineGun("DARE TO BE DIFFERENT DARE TO BE ISTE", 1);
